...
const char *stringArray[5] = {
    "Mouse Manufacturer", "Automatic Mouse Mover", "Mouse-1337", "Mouse Configuration", "Mouse Interface",
};
...
class HidMouseInterface : public UsbInterface {
    public:
        HidMouseInterface(uint8_t bInterfaceNumber, uint8_t bNumEndpoints, UsbEndpoint **endpointArray, uint8_t iInterface)
...
class HidMouse : public UsbDevice {
    public:
        HidMouse(uint16_t vid, uint16_t pid, uint16_t bcdVer) : UsbDevice(vid, pid, bcdVer, 0, 0, 0, 1, configurationList, &usbString, 1, 2, 3){};
